...
describe("WithdrawalService", () => {
  it("creates an item_removed movement and refreshes stock in a transaction", async () => {
    ...
    expect(calls).toContainEqual({
      ...
      args: {
        data: {
          idempotencyKey: expect.stringMatching(/^inventory\.withdrawal\.created:staff-1:/),
  ...
  it("runs the withdrawal transaction at the serializable isolation level", async () => {
    const calls: Array<{ model: string; method: string; args?: unknown }> = [];
    const now = new Date("2026-05-26T08:00:00.000Z");
    const tx = withdrawalTransaction({
      calls,
    ...
    await service.create(
      { inventoryItemId: "item-1", quantity: 3, unit: "Stück" },
      { userId: "staff-1", role: "staff" }
    );

    expect(capturedOptions?.isolationLevel).toBe("Serializable");
  });
