...
import type { BarRefillServicePort } from "../src/modules/inventory/bar-refill.service.js";
...
import type { InventoryMasterDataDto } from "../src/modules/inventory/inventory-master-data.service.js";
import type { InventoryReadServicePort } from "../src/modules/inventory/inventory-read.service.js";
import type { PurchaseOrderReadDto } from "../src/modules/inventory/inventory.schemas.js";
...
import type { TransferServicePort } from "../src/modules/inventory/transfer.service.js";
import type { WithdrawalServicePort } from "../src/modules/inventory/withdrawal.service.js";
import { InventoryConflictError } from "../src/modules/inventory/errors.js";
...
describe("inventory API routes", () => {
  ...
  it("forces demo mode off in production even when DEMO_MODE is requested", async () => {
    const seedCalls: string[] = [];
    const app = buildApp({
      env: {
        NODE_ENV: "production",
    ...
    try {
      ...
      const response = await app.inject({ method: "GET", url: "/app-context" });
  ...
  it("passes the authenticated organization to operational inventory master data", async () => {
    const calls: string[] = [];
    const app = buildApp({
      inventory: fakeInventoryServices({
        inventoryMasterDataService: {
          async list(actorOrganizationId: string) {
            calls.push(actorOrganizationId);
    ...
    try {
      ...
      expect(response.statusCode).toBe(200);
      expect(calls).toEqual([testOrganizationId]);
    } finally {
      await app.close();
    }
  });
  ...
  it("lets operational actors record transfers and passes actor context", async () => {
    for (const role of ["admin", "shift_lead", "staff"] as const) {
      const calls: Array<{ input: unknown; actor: Actor }> = [];
      const app = buildApp({
        inventory: fakeInventoryServices({
      ...
      try {
        ...
        expect(calls[0]).toEqual({
          ...
          actor: expect.objectContaining({
            role: role === "admin" ? "admin" : role === "shift_lead" ? "shift_lead" : "staff"
          })
        });
      } finally {
        await app.close();
  ...
  it("rejects viewers from recording transfers", async () => {
    const app = buildApp({
      inventory: fakeInventoryServices()
    });
    ...
    try {
      ...
      const response = await app.inject({
        method: "POST",
        url: "/transfers",
        headers: authHeaders("viewer-1", "viewer"),
        payload: {
          inventoryItemId: "item-1",
          quantity: 3,
          unit: "Stück",
  ...
  it("surfaces org-mismatch transfer conflicts without mutating state", async () => {
    const app = buildApp({
      inventory: fakeInventoryServices({
        transferService: {
          async create() {
    ...
    try {
      ...
      expect(response.json()).toEqual({
        ...
        message: "target storage location organization mismatch"
      });
    } finally {
      await app.close();
    }
  ...
  it("lets shift leads approve correction requests they did not raise", async () => {
    const calls: Array<{ id: string; actor: Actor }> = [];
    const app = buildApp({
      inventory: fakeInventoryServices({
        correctionService: {
  ...
  it("prevents staff from approving or rejecting correction requests", async () => {
    for (const action of ["approve", "reject"] as const) {
      ...
      try {
        ...
        const response = await app.inject({
          method: "POST",
          url: `/admin/correction-requests/correction-1/${action}`,
          headers: authHeaders("staff-1", "staff")
        });
  ...
  it("lets shift leads reject correction requests they did not raise", async () => {
    const calls: Array<{ id: string; actor: Actor }> = [];
    const app = buildApp({
      inventory: fakeInventoryServices({
        correctionService: {
  ...
  it("blocks inventory reset for admins when demo mode is disabled", async () => {
    const app = buildApp({
      inventory: fakeInventoryServices()
    });
  ...
  it("creates or returns the current bar refill run for operational roles", async () => {
    const app = buildApp({
      inventory: fakeInventoryServices()
    });
    ...
    try {
      ...
      expect(response.statusCode).toBe(200);
      expect(response.json()).toEqual(expectedBarRefillRun());
    } finally {
      await app.close();
    }
  });

  it("updates requestedQuantity and returns the run payload", async () => {
    const app = buildApp({
      inventory: fakeInventoryServices()
    });
    ...
    try {
      ...
      const response = await app.inject({
        method: "PATCH",
        url: "/bar-refill/runs/refill-run-1/items/refill-item-1",
        headers: authHeaders("staff-1", "staff"),
        payload: {
          requestedQuantity: 2
        }
  ...
  it("confirms bar refill item and returns movement linkage", async () => {
    const app = buildApp({
      inventory: fakeInventoryServices()
    });
    ...
    try {
      ...
      const response = await app.inject({
        method: "POST",
        url: "/bar-refill/runs/refill-run-1/items/refill-item-1/confirm",
        headers: authHeaders("shift-1", "shift_lead")
      });

      expect(response.statusCode).toBe(200);
      expect(response.json()).toEqual({
        run: expectedBarRefillRun({
          status: "completed",
          itemStatus: "confirmed",
          requestedQuantity: 2
        }),
        movementId: "move-refill-1",
        stockAfter: 9
      });
    } finally {
      await app.close();
  ...
  it("rejects bar refill routes for unauthorized roles", async () => {
    const app = buildApp({
      inventory: fakeInventoryServices()
    });
...
function fakeInventoryServices(
  ...
  return {
    ...
    transferService: overrides.transferService ?? {
      async create() {
        return { movementId: "move-4", stockFromAfter: 4, stockToAfter: 6 };
      }
    } as TransferServicePort,
    ...
    barRefillService: overrides.barRefillService ?? {
      async getTodayRun() {
        return expectedBarRefillRun();
      },
      async createOrGetTodayRun() {
        return expectedBarRefillRun();
      },
      async getRun() {
        return expectedBarRefillRun();
      ...
      async confirmRunItem() {
        return {
          run: expectedBarRefillRun({ status: "completed", itemStatus: "confirmed", requestedQuantity: 2 }),
          movementId: "move-refill-1",
          stockAfter: 9
        };
      },
      async cancelRunItem() {
        return expectedBarRefillRun({ status: "open", itemStatus: "cancelled", requestedQuantity: 0 });
      }
    } as BarRefillServicePort,
...
function expectedPurchaseOrderReadModel(): PurchaseOrderReadDto {
  return {
    purchaseOrderId: "po-1",
    status: "ordered",
...
function expectedMasterData(): InventoryMasterDataDto {
  return {
    suppliers: [
      {
        supplierId: "supplier-1",
...
function expectedBarRefillRun(overrides?: {
  status?: "open" | "partially_confirmed" | "completed";
  itemStatus?: "open" | "pending" | "confirmed" | "cancelled";
  requestedQuantity?: number;
}) {
  return {
    runId: "refill-run-1",
    organizationId: testOrganizationId,
    runDateLocal: "2026-06-01",
    timezone: "Europe/Berlin",
    createdBy: "staff-1",
    status: overrides?.status ?? "open",
    createdAt: "2026-06-01T08:00:00.000Z",
    completedAt: overrides?.status === "completed" ? "2026-06-01T09:00:00.000Z" : undefined,
    items: [
      {
        id: "refill-item-1",
        templateItemId: "bar-template-1",
        inventoryItemId: "item-1",
        displayOrder: 1,
        ...
        stockMovementId: overrides?.itemStatus === "confirmed" ? "move-refill-1" : undefined
      }
    ]
  };
}
