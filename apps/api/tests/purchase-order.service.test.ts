...
describe("PurchaseOrderService", () => {
      ...
      db: {
        ...
        supplier: {
          async findFirst(args: unknown) {
            calls.push({ model: "supplier", method: "findFirst", args });
  ...
  it("rejects purchase orders that reference another organization's supplier", async () => {
    const service = new PurchaseOrderService({
      db: {
        purchaseOrder: {
          async create() {
    ...
    await expect(
      ...
      message: "supplier organization mismatch",
      statusCode: 409
    });
  });
