...
describe("Inventory Prisma schema contract", () => {
  ...
  it("keeps InventoryMovement as the inventory source-of-truth log", () => {
    ...
    expectLine(block, 'barRefillRunItemId String?              @unique @map("bar_refill_run_item_id")');
    expectLine(block, 'fromStorageLocationId String?            @map("from_storage_location_id")');
    expectLine(block, 'toStorageLocationId String?              @map("to_storage_location_id")');
  ...
  it("adds model-backed organization ownership columns for items and locations", () => {
    const inventoryItemBlock = getBlock("model", "InventoryItem");
    const storageLocationBlock = getBlock("model", "StorageLocation");

    expectLine(inventoryItemBlock, 'organizationId    String?          @map("organization_id")');
    expectLine(inventoryItemBlock, 'categoryId        String?          @map("category_id")');
    expectLine(inventoryItemBlock, 'targetStock       Float?           @map("target_stock")');
    expectLine(inventoryItemBlock, 'displayOrder      Int?             @map("display_order")');
    expectLine(inventoryItemBlock, "@@index([organizationId])");
    expectLine(inventoryItemBlock, "@@index([organizationId, id])");
    expectLine(inventoryItemBlock, "@@index([categoryId])");

    expectLine(storageLocationBlock, 'organizationId   String?         @map("organization_id")');
    expectLine(storageLocationBlock, "@@index([organizationId])");
    expectLine(storageLocationBlock, "@@index([organizationId, id])");
  });

  it("adds optional organization ownership for suppliers", () => {
    const supplierBlock = getBlock("model", "Supplier");

    expectLine(supplierBlock, 'organizationId String?  @map("organization_id")');
    expectLine(supplierBlock, "@@index([organizationId])");
  });
  ...
  it("defines bar refill status enums", () => {
    const runStatus = getBlock("enum", "BarRefillRunStatus");
    const itemStatus = getBlock("enum", "BarRefillRunItemStatus");

    for (const status of ["open", "partially_confirmed", "completed"]) {
      expectLine(runStatus, status);
    }
    for (const status of ["open", "pending", "confirmed", "cancelled"]) {
      expectLine(itemStatus, status);
    }
  });
});
