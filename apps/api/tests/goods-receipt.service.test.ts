...
describe("GoodsReceiptService", () => {
  it("creates receipt, movement, event, and unlinked staff review in a transaction", async () => {
    ...
    expect(calls).toContainEqual({
      ...
      args: {
        data: {
          idempotencyKey: "inventory.goods_receipt.item_recorded:gri-1",
  ...
  it("updates linked purchase order quantities and creates overdelivery review", async () => {
    ...
    const tx = {
      ...
      inventoryMovement: {
        ...
        async findMany() {
          return [{ type: "goods_received" as const, quantity: 12, createdAt: receivedAt }];
        }
      },
      inventoryStockSnapshot: {
        async upsert() {
...
function receiptTransaction(input: {
  ...
  return {
    ...
    inventoryMovement: {
      ...
      async findMany() {
        return [{ type: "goods_received" as const, quantity: 4, createdAt: input.receivedAt }];
      }
    },
    inventoryStockSnapshot: {
      async upsert(args: unknown) {
