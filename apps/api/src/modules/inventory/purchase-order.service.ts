...
export type PurchaseOrderDatabaseClient = {
  ...
  supplier?: {
    findFirst(args: {
      where: {
        id: string;
        OR: Array<{ organizationId: string } | { organizationId: null }>;
      };
      select: {
        id: true;
      };
...
export type PurchaseOrderServicePort = {
  create(input: CreatePurchaseOrderInput, actor: Pick<Actor, "organizationId" | "userId">): Promise<PurchaseOrderDto>;
...
export class PurchaseOrderService implements PurchaseOrderServicePort {
  ...
  public async create(
    ...
    await this.assertSupplierAccessible(input.supplierId, actor.organizationId);
  ...
  private async assertSupplierAccessible(
    ...
    if (!this.options.db.supplier) {
      throw new Error("supplier read model is not available");
    }

    const supplier = await this.options.db.supplier.findFirst({
      where: {
        id: supplierId,
        OR: [{ organizationId: actorOrganizationId }, { organizationId: null }]
      },
      select: {
        id: true
      }
    ...
    if (!supplier) {
      throw new InventoryConflictError("supplier organization mismatch");
    }
  }
}
