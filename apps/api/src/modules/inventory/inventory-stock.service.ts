...
export class InventoryStockService {
  ...
  public async calculateStock(input: {
    ...
    return movements.reduce(
      (total, movement) => total + signedQuantity(movement, input.storageLocationId),
      0
    );
  }
...
export function signedQuantity(
  movement: Pick<
    ...
    "type" | "quantity" | "storageLocationId" | "fromStorageLocationId" | "toStorageLocationId"
  >,
  storageLocationId?: string
): number {
  if (movement.type === "transfer") {
    ...
    if (movement.fromStorageLocationId === storageLocationId) {
      return -movement.quantity;
    }
    if (movement.toStorageLocationId === storageLocationId) {
      return movement.quantity;
    }
    return 0;
  }

  if (storageLocationId && movement.storageLocationId !== storageLocationId) {
    return 0;
  }
