...
export const createTransferSchema = z
  ...
  .refine((input) => input.fromStorageLocationId !== input.toStorageLocationId, {
    message: "fromStorageLocationId and toStorageLocationId must differ",
...
const nonNegativeNullableNumber = z.number().nonnegative().nullable();

export const updateBarRefillRunItemSchema = z
  .object({
    requestedQuantity: nonNegativeNullableNumber.optional()
  })
  .refine((input) => Object.prototype.hasOwnProperty.call(input, "requestedQuantity"), {
    message: "requestedQuantity is required"
  })
  .transform((input) => ({
...
export const importInventoryCsvSchema = z.object({
  csv: z
    .string()
    .min(1)
    .refine((value) => Buffer.byteLength(value, "utf8") <= maxCsvBytes, {
      message: `csv payload must not exceed ${maxCsvBytes} bytes`
    })
    .refine(
      (value) => value.split(/\r\n|\r|\n/).filter((line) => line.trim() !== "").length <= maxCsvRows + 1,
      {
        message: `csv must not exceed ${maxCsvRows} data rows`
      }
    ),
...
export type UpdateBarRefillRunItemInput = z.infer<typeof updateBarRefillRunItemSchema>;
...
export type BarRefillRunItemStatus = "open" | "pending" | "confirmed" | "cancelled";

export type BarRefillRunItemDto = {
  id: string;
...
export type BarRefillRunDto = {
  ...
  createdAt: string;
  completedAt?: string;
  items: BarRefillRunItemDto[];
};

export type BarRefillItemConfirmDto = {
  run: BarRefillRunDto;
  movementId: string;
  stockAfter: number;
};
