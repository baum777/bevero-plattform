-- Fix: ON CONFLICT must not overwrite displayName/preferredStorageLocationId with NULL.
-- Prior behavior: calling upsert with display_name=NULL silently nullified the existing value.
-- New behavior: COALESCE keeps the existing value when the caller passes NULL.
