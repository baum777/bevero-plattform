...
// Auth model: the backend (src/modules/auth/actor.ts) only accepts a signed
// HS256 bearer JWT whose `sub` maps to an organization member with an admin
// role. A pre-signed SMOKE_JWT_TOKEN is used verbatim when provided; otherwise
// we mint a token for a scoped smoke owner and seed/teardown its membership.
const appJwtSecret = process.env.SUPABASE_JWT_SECRET?.trim() || "test-supabase-jwt-secret";
const providedSmokeToken = process.env.SMOKE_JWT_TOKEN?.trim();
const smokeActorUserId = `${smokePrefix}admin-${randomUUID()}`;
const smokeActorMemberId = `member_${randomUUID()}`;
const smokeActorOrganizationId = `${smokePrefix}org-${randomUUID()}`;
const seedSmokeMembership = !providedSmokeToken;
const smokeBearerToken = providedSmokeToken ?? createSmokeJwt(smokeActorUserId, appJwtSecret);
const adminHeaders = {
...
async function seedSmokeActorMembership(): Promise<void> {
  ...
  console.log(`Seeded smoke admin membership: ${smokeActorUserId}`);
}
...
async function teardownSmokeActorMembership(): Promise<void> {
  ...
  try {
    ...
    console.error("Failed to remove smoke admin membership:");
    console.error(formatError(error));
  }
}
